...
def square(i, j):
    ...
    return [V(i + k[0], j + k[1]) for k in w]


def sub_sqr():
    pos = [(0, 0), (0, 3), (0, 6), (3, 3), (3, 6), (6, 6), (3, 0), (6, 0)]
    return [all_different(square(p[0], p[1])) for p in pos]           
def get_raw(i):
...
def sudoku(assigments):
    ...
    cs = domains(variables) + vertical() + horizontal() + sub_sqr()#TODO: too weak contraints, add something!
