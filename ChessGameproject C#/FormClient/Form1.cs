    ...
    public partial class Form1 : Form
    {
        ...
        private void Form_Load(object sender, EventArgs e)
        {
            for (int x = 1; x < boardLayoutPanel.ColumnCount; x++)
            {
                for (int y = 1; y < boardLayoutPanel.RowCount; y++)
                {
                    ...
                    if ((x + y) % 2 == 1) button.BackColor = Color.LightGray;
        ...
        private void Click_Board(object s, EventArgs e)
        {
            ...
            if (!(button.Tag is ChessPiece))
            {
                if (selectedPlayer > -1)
                {
                    chessBoard.MakePieceAction(selectedPiece.x, selectedPiece.y, a.Column - 1, a.Row - 1);
                    selectedPlayer = -1;
                    DrawPieces(chessBoard);
                }
                return;
            ...
            else
            {
                ...
                foreach (Chess.Point point in chessBoard.PosiblePieceActions(a.Column - 1, a.Row - 1))
                {
